...
const AnyDownloader = ({ title }) => {
  ...
  const fbDownloader = async () => {
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BACKEND_URL}/fb-video-download`,
        { url }
      );
  ...
  const xDownloader = async () => {
    try {
      ...
      if(isValid){
        const response = await axios.post(
          `${import.meta.env.VITE_BACKEND_URL}/x-video-download`,
          { url },
        );
  ...
  const linkedinDownloader = async () => {
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BACKEND_URL}/linkedin-video-download`,
        { url },
      );
  ...
  const titktokDownloader = async () => {
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BACKEND_URL}/tiktok-video-download?url=${url}`
      );
  ...
  return (
    <div className="mt-[72px] dark:bg-[#0f172a]">
      ...
      <p className="text-center text-gray-500 my-5">Download from link!</p>
      <div className="w-[90%] sm:w-[80%] md:w-[70%] lg:w-[60%] mx-auto">
        <input
          type="text"
          placeholder={`Enter ${title} URL...`}
          className="w-full border-2 outline-none border-[#1A8FE3] p-3 px-5 rounded-full text-black dark:text-white dark:bg-[#0f172a] dark:border-white accent-[#1A8FE3]"
          value={url}
