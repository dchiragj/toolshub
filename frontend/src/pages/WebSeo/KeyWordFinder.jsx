    ...
    return (
        <div className='mt-[72px] dark:bg-[#0f172a]'>
            ...
            <div className='flex justify-center items-center mt-5'>
                <button className={`px-5 py-2 ${!text ? "bg-gray-500" : "bg-[#1A8FE3]"} text-white rounded-lg`} disabled={!text} onClick={findKeyword}>Find Keyword</button>
            </div>
            {
                loading &&
            ...
            }
            {
                ...
                <div className='w-[90%] sm:w-[80%] md:w-[70%] lg:w-[60%] mx-auto mt-5 flex flex-col justify-center items-center gap-5 dark:text-white'>
                    ...
                    <div className='flex flex-wrap justify-center items-center gap-5'>
                        <button className={`px-5 py-2 bg-[#1A8FE3] text-white rounded-lg`} onClick={findKeyword}>Regenerate</button>
                        <CopyToClipboard text={keyWord} onCopy={() => setCopy(true)}>
                            <button className={`px-5 py-2 bg-[#1A8FE3] text-white rounded-lg`}>Copy</button>
                        </CopyToClipboard>
                        <button className={`px-5 py-2 bg-[#1A8FE3] text-white rounded-lg`} disabled={!text} onClick={() => { setText(""); setKeyWord(null) }}>Reset</button>
                    </div>
                </div>
            }
            <FreeTools />
