...
const RenderPageFromPDF = () => {
    ...
    return (
        <div className='mt-[72px] dark:bg-[#0f172a]' >
            ...
            <p className='text-center text-gray-500 my-5'>Easily extract any page from pdf documents online for free.</p>
                ...
                {file &&
                    <>
                        ...
                        <input
                            ...
                            className='p-2 border-2 border-[#1A8FE3] rounded-full'
                            value={pageNo}
                            onChange={(e) => setPageNo(Number(e.target.value))}
                        />
                        <div className='flex justify-center items-center gap-5'>
                            <button className={`mx-auto ${!pageNo ? "bg-gray-500" : "bg-[#1A8FE3]"} px-6 py-3 text-white rounded-lg`} disabled={!pageNo} onClick={renderPage}>Render Page No : {pageNo}</button>
                            <button className={`mx-auto ${!renderedPage ? "bg-gray-500" : "bg-[#1A8FE3]"} px-6 py-3 text-white rounded-lg`} disabled={!renderedPage} onClick={handleDownload}>Download Page : {renderedPage}</button>
                        </div>
                        {
