...
const LinkShortener = () => {
    ...
    const handleShortURL = async () => {
        alert(import.meta.env.VITE_TINY_URL)
        try {
            const response = await axios.post('https://api.tinyurl.com/create', {
                ...
                headers: {
                    ...
                    'Authorization': `Bearer ${import.meta.env.VITE_TINY_URL}`,
                },
            });
    ...
    return (
        <div className='mt-[72px] dark:bg-[#0f172a]'>
            ...
            <div className='flex flex-col justify-center items-center mt-5 gap-5 dark:text-white'>
                <button className={`px-5 py-2 ${!url ? "bg-gray-500" : "bg-[#1A8FE3]"} text-white rounded-lg`} disabled={!url} onClick={handleShortURL}>Shorten URL</button>
                {
                    shortURL &&
                    <>
                        ...
                        <div className='flex justify-center items-center gap-5'>
                            ...
                            <button className={`px-5 py-2 bg-[#1A8FE3] text-white rounded-lg`} onClick={() => { setURL(""); setShortURL(""); setCopy(false) }}>Reset</button>
                        </div>
                    </>
                }
            </div>
