...
const Sidebar = ({ expandSidebar, setExpandSidebar }) => {
  ...
  return (
    <>
      <div
        className={`w-[80%] bg-white sm:w-[50%] dark:bg-[#0f172a] flex flex-col gap-2 lg:hidden p-4 shadow-xl absolute ${expandSidebar ? 'left-0' : '-left-[500px]'} top-[72px] overflow-y-auto`}
        style={{ height: "calc(100vh - 72px)" }}
      >
        <li className="list-none outline-none rounded-lg mb-3">
          <p className="mb-3 text-black dark:text-gray-200 text-[15px]">Select Language</p>
          ...
          sideBarTools.map((val, ind) => {
            return (
              <Accordion key={ind} expanded={expanded === val.panel} onChange={handleChange(val.panel)} sx={{borderRadius: "12px !important",backgroundColor: darkMode ? "#1a252e":"white",color: darkMode?"white":""}} className="bg-black">
                ...
                <AccordionDetails>
                  <p className={`text-sm text-gray-200 mb-3`}>{val.title1}</p>
                  <div className="flex flex-col">
                    {val.option1?.map((val, ind) => {
                      return (
                        <NavLink key={ind} onClick={toggleSideBar} className={"w-full p-3 hover:bg-[aliceblue] hover:text-[#1A8FE3] rounded-lg"} to={val.link}>
                          {val.title}
                        </NavLink>
                      )
                    })}
                  </div>
                  {
                    ...
                    <>
                      <p className="text-sm text-gray-200 my-3">{val.title2}</p>
                      <div className="flex flex-col">
