...
const CommonPage = ({ name }) => {
    ...
    return (
        <div className='mt-[72px] dark:bg-[#0f172a] h-full'>
            <CommonPageHeader />
            <Ads slot='' className="mx-10 my-5" />
            ...
            <div className='pb-16 pt-2'>
                <div
                    className={`w-[90%] h-[400px] bg-[#0f172a] p-20 text-center mx-auto text-white flex flex-col justify-center items-center gap-14 dark:border dark:border-dashed`}
                    {...getRootProps()}
                >
                        ...
                        <div className='flex justify-center items-center gap-5'>
                            {
                                ...
                                file ?
                                    <div className='flex justify-center items-center gap-5'>
                                        <button
                                            className='border border-solid border-gray-500 bg-transparent text-[#4ca49f] hover:bg-[#4ca49f] hover:text-white px-5 py-3.5 sm:py-3 flex justify-center items-center gap-3 rounded text-xs sm:text-base'
                                        ...
                                        <button
                                            className='border border-solid border-gray-500 bg-[#4ca49f] text-[white] hover:bg-transparent hover:text-[#4ca49f] px-5 py-3.5 sm:py-3 flex justify-center items-center gap-3 rounded text-xs sm:text-base'
                                            title='Compress File'
                            ...
                            {
                                !file && <button
                                    className='border border-solid border-gray-500 bg-transparent text-[white] hover:bg-[#4ca49f] hover:text-[white] p-2 px-3 flex justify-center items-center gap-3 rounded'
                    ...
                    {linkUploadClicked && !showProgress && <div className='flex justify-center items-center border border-gray-500' onClick={(e) => e.stopPropagation()}>
                        <div className='h-full p-2'>
                            <TbWorldWww className='' fontSize={28} />
                        </div>
                        <input type="text" placeholder='Paste Link Here...' className='p-2 w-52 h-full text-white outline-none' autoFocus value={url} onChange={(e) => setUrl(e.target.value)} />
...

                    {
                        ...
                        <div className='w-full sm:w-[80%] md:w-[70%] lg:w-[60%] xl:w-[50%] flex flex-col justify-center items-center gap-5' >
                            ...
                            <button className='bg-[#1A8FE3] px-5 py-2 rounded-lg text-lg' onClick={handleCompress}>Compress File</button>
                            <p>Your file size can reduced up to {progressValue}% of your origional Size</p>
                        </div>
                    }
                    {
                        compressedFile && <button className='bg-[#1A8FE3] px-5 py-2 rounded-lg text-lg' onClick={handleDownload}>Download File</button>
                    }
                </div>
            </div>
