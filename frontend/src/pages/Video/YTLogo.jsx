...
const YTLogo = () => {
    ...
    const getLogo = async () => {
        try {
            const response = await fetch(`https://www.googleapis.com/youtube/v3/channels?part=snippet&id=${channelId}&key=${import.meta.env.VITE_YT_API_KEY}`);
    ...
    return (
        <div className='mt-[72px] dark:bg-[#0f172a]'>
            ...
            <p className='text-center text-gray-500 my-5'>Easily download youtube channel logo online for free.</p>
            <div className='w-[90%] sm:w-[80%] md:w-[70%] lg:w-[60%] mx-auto flex flex-col justify-center items-center gap-5'>
                ...
                <input
                    ...
                    className='w-full border-2 border-[#1A8FE3] outline-none rounded-full p-2'
                    value={channelId}
                    onChange={(e) => setChannelId(e.target.value)}
                />
                <button disabled={!channelId} className={`${channelId ? "bg-[#1A8FE3]" : "bg-gray-500"} text-white px-4 py-2 rounded-lg`} onClick={getLogo}>Get Logo</button>
                {
                    ytLogo &&
                    <>
                        <img src={ytLogo} alt="YouTube Logo" className='w-[200px]' />
                        <button className='bg-[#1A8FE3] text-white rounded-lg px-4 py-2' onClick={handleDownload}>Download</button>
                    </>
                }
            </div>
            <FreeTools />
