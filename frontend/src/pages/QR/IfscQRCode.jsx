...
const IfscQRCode = () => {
    ...
    return (
        <div className='mt-[72px] dark:bg-[#0f172a]'>
            ...
            <p className='text-center text-gray-500 my-5'>Easily IFSC Code For Indian Bank Details online for free.</p>
            <div className='grid grid-cols-1 lg:grid-cols-2 gap-10 m-10'>
                <div className='flex flex-col justify-center items-center gap-5'>
                    ...
                    <input
                        type="text"
                        className='w-[90%] sm:w-[80%] mx-auto border-2 outline-none border-[#1A8FE3] p-2 px-5 rounded-full text-black dark:text-white dark:bg-[#0f172a] dark:border-white'
                        value={account.name}
                    ...
                    />
                    <button className={`${!account.name || !account.AcNumber || !account.ifsc ? "bg-gray-500" : "bg-[#1A8FE3]"} text-white rounded-lg  px-4 py-2`} disabled={!account.name || !account.AcNumber || !account.ifsc} onClick={generate}>Generate Barcode</button>
                </div>
                <div className='flex flex-col justify-center items-center gap-5'>
                    {
                        qrValue ?
                            <>
                                ...
                                <QRCodeCanvas
                                    ...
                                    size={256}
                                />
                                <button className='bg-[#1A8FE3] text-white px-4 py-2 rounded-lg' onClick={handleDownload}>Download QR</button>
                            </>
