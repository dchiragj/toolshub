    ...
    return (
        <div className='mt-[72px] dark:bg-[#0f172a]'>
            ...
            <div className='w-[90%] sm:w-[80%] md:w-[70%] lg:w-[60%] mx-auto flex flex-col justify-center items-center gap-3'>
                ...
                <button className={`${metaDetails.description ? "bg-[#1A8FE3]" : "bg-gray-500"} rounded-lg px-4 py-2 text-white mt-3`} disabled={!metaDetails.description} onClick={generateMeta}>Generate</button>
                {
                    generatedMeta &&
                    <div className='border border-gray-200 shadow-custom flex flex-col justify-center items-center p-5 rounded-lg'>
                        {generatedMeta.split('\n').map((line, index) => (
                            <div key={index}>{line}</div>
                        ))}
                        <div className='flex justify-center items-center gap-5'>
                            <CopyToClipboard text={generatedMeta} onCopy={() => setCopy(true)}>
                                <button className={`bg-[#1A8FE3] rounded-lg px-4 py-2 text-white mt-3`}>Copy</button>
                            </CopyToClipboard>
                            <button className={`bg-[#1A8FE3] rounded-lg px-4 py-2 text-white mt-3`} onClick={() => { setMetaDetails(metaObj); setGeneratedMeta(''); }}>Reset</button>
                        </div>
                    </div>
                }
            </div>
