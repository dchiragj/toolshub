...
const AnyTextTypeToPDF = ({ type }) => {
    ...
    return (
        <div className='mt-[72px] dark:bg-[#0f172a]'>
            ...
            <p className='text-center text-gray-500 my-5'>Convert {type.toUpperCase()} to PDF online for free</p>
            ...
            <div className='w-[90%] sm:w-[80%] md:w-[70%] lg:w-[60%] mx-auto flex flex-col justify-center items-center gap-5'>
                ...
                ></textarea>
                <button className={`px-5 py-2 ${!file && !text ? "bg-gray-500" : "bg-[#1A8FE3]"} text-white rounded-lg`} disabled={!file && !text} onClick={convert}>Generate PDF</button>
                {
                    pdfUrl &&
                    <button className={`px-5 py-2 bg-[#1A8FE3] text-white rounded-lg`} disabled={!file && !text} onClick={handleDownload}>Download</button>
                }
            </div>
            <FreeTools />
            <WebTools />
