            ...
            <CommonPageHeader />
            {
                from !== '' && to !== '' ?
                    <>
                        ...
                        <p className='text-center text-gray-500 my-5'>Easily convert {from} to {to} online for free.</p>
                        <input type="file" accept={`${fromFileType}`} onChange={handleChange} className='hidden' />
                        <div className='flex flex-col justify-center items-center gap-5'>
                            <button className=' mx-auto bg-[#1A8FE3] px-6 py-3 text-white rounded-lg' onClick={() => document.querySelector('input[type="file"]').click()}>Choose File</button>
                            {
                                file &&
                                <p className='text-center dark:text-white'>Selected : {file.name}</p>
                            }
                            {
                                convertedFile ? <button className=' mx-auto bg-[#1A8FE3] px-6 py-3 text-white rounded-lg' onClick={handleDownload}>Download File</button> :
                                    <button className={`mx-auto ${!file ? "bg-gray-500" : "bg-[#1A8FE3]"} px-6 py-3 text-white rounded-lg`} disabled={!file} onClick={convert}>Convert to {to}</button>
                            }
                        </div>
                    </> :
                    <>
                        ...
                        <p className='text-center text-gray-500 my-5'>Easily convert markdown online for free.</p>
                        <div className='w-[90%] sm:w-[80%] mx-auto'>
                            <textarea
                                rows={5}
                                className='w-full border-2 border-[#1A8FE3] rounded-lg p-3'
                        ...
                        <div className='flex flex-wrap justify-center items-center gap-5 mt-5'>
                            <button className={`${!text ? "bg-gray-500" : "bg-[#1A8FE3]"} px-6 py-3 text-white rounded-lg`} disabled={!text} onClick={convertToMD}>Convert to Markdown</button>
                            <button className={`${!text ? "bg-gray-500" : "bg-[#1A8FE3]"} px-6 py-3 text-white rounded-lg`} disabled={!text} onClick={convertToHTML}>Convert to HTML</button>
                            <CopyToClipboard text={convertedText} onCopy={() => setCopyText(true)}>
                                <button className={`${!convertedText ? "bg-gray-500" : "bg-[#1A8FE3]"} px-6 py-3 text-white rounded-lg`} disabled={!convertedText}>Copy</button>
                            </CopyToClipboard>
                            <button className={`${!convertedText ? "bg-gray-500" : "bg-[#1A8FE3]"} px-6 py-3 text-white rounded-lg`} disabled={!convertedText} onClick={() => { setText(""); setConvertedText("") }}>Reset</button>
                        </div>
                        {
                            convertedText &&
