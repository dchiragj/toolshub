...
const Base64ToPdf = ({ from, to }) => {
    ...
    return (
        <div className='mt-[72px] dark:bg-[#0f172a]'>
            ...
            <p className='text-center text-gray-500 my-5'>Easily convert {from.toUpperCase()} to {to.toUpperCase()} documents online.</p>
            <div className='w-[90%] sm:w-[80%] md:w-[70%] lg:w-[60%] mx-auto flex flex-col justify-center items-center gap-5'>
                ...
                {
                    to === 'pdf' ?
                        <button className={`px-5 py-2 ${!base64 ? "bg-gray-500" : "bg-[#1A8FE3]"} text-white rounded-lg`} disabled={!base64} onClick={convertToPDF}>Generate PDF</button> :
                        <button className={`px-5 py-2 ${!file ? "bg-gray-500" : "bg-[#1A8FE3]"} text-white rounded-lg`} disabled={!file} onClick={convertToBase64}>Generate Base64</button>
                }
                {
                    pdfUrl &&
                    <button className={`px-5 py-2 bg-[#1A8FE3] text-white rounded-lg`} onClick={handleDownload}>Download PDF</button>
                }
                {
                    text &&
                    <>
                        ...
                        <CopyToClipboard text={text} onCopy={() => setCopy(true)}>
                            <button className={`px-5 py-2 bg-[#1A8FE3] text-white rounded-lg`}>Copy Base64</button>
                        </CopyToClipboard>
                    </>
                }
            </div>
